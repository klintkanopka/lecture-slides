...
- What about finding groups of people?
  ...
  - Clustering, inherently, is the task of assigning _rows_ to discrete groups based on similarities in their values on the columns
  - With item response data, clustering puts people into groups with similar response patterns
  - There are _ton_ of ways to do this task, today we talk about "model-free" methods
...
- Clusters are described by _centroids_, or their center point
...
- We follow a two step process:
  - First, we assign each point to a cluster by finding the centroid that it’s closest to
  - Second, we move each centroid to the middle of the points assigned to it
- We just do this over and over until convergence (the centroids stop moving)
...
2. Assign each observation from the dataset to the centroid that it is closest to using the Euclidean distance
